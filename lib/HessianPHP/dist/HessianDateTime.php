...
class HessianDateTime{
	 ...
	 **/
	function HessianDateTime($date='now'){
		//echo $date;
		if(is_string($date)) {
		...
		elseif(is_object($date) && is_a($date,'HessianDateTime'))
	...
	function &asTime(){
		return new HessianDateTime($this->getTime());
	}

	function &asDate() {
		return new HessianDateTime($this->getDate());
	}
	...
	function isBefore($time){
		if(HessianDateTime::compare($this,$time) == -1)
			return true;
		return false;
	}
	...
	function equals($time){
		if(HessianDateTime::compare($this,$time) == 0)
			return true;
		return false;
	}

	function getHessianDateTime(){
	...
	/**
	 * Calculates the difference between two HessianDateTime objects.
	 * Returns an associative array containing a timespan expressed in days, hours, minutes and seconds.
	 *  
	 * @param HessianDateTime other Object to compare with
	 ...
	 * Can work with HessianDateTime objects, string values and integer timestamps
	 *
	...
	function compare($time1,$time2){
		...
		$ts1 = HessianDateTime::findTimestamp($time1);
		$ts2 = HessianDateTime::findTimestamp($time2);
	...
	function monthName($value=null){
		...
		}elseif(is_object($value) && is_a($value,'HessianDateTime'))
			$ts = $value->_timestamp;
		else {
	...
	function findTimestamp($dateObj){
		if(is_object($dateObj) && is_a($dateObj,'HessianDateTime'))
	...
	function __dateOperation($op,$part,$num){
		...
		if(!in_array($part,$parts)) {
			return trigger_error("operation: '$part' is not a part of HessianDateTime",E_USER_WARNING);
		}
		switch($op){
			case 'add':
	...
	function __toString(){
		return $this->getHessianDateTime();
	}

}

if (function_exists('overload') && phpversion() < 5) {
   overload('HessianDateTime');
}

/*
// Tests

$date1 = new HessianDateTime('2004-09-06 8:00:00');
$date2 = new HessianDateTime('2004-09-06 9:00:00');
$date3 = new HessianDateTime('2004-09-06 8:00:00');
...
$now = new HessianDateTime();
echo $now->monthName();
echo $now->monthName($date1);
echo HessianDateTime::monthName($now);
echo '<br>';
echo $now->getHessianDateTime();
$now->addYears(5);
echo '<br>';
echo $now->getHessianDateTime();
